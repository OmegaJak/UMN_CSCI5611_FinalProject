...
// (348 Hz)
double k = 50000, kV = 0.3, tension = .848, length = 1.755;
...
void audio_callback(void* _beeper, Uint8* _stream, int _len) {
  ...
  for (i = 0; i < len && (lastP < lastS); i++) {
    ...
    amp = amp > 3 * ToneVolume
              ? 3 * ToneVolume
              : amp < -3 * ToneVolume ? -3 * ToneVolume : amp;  // clamp
    stream[i] = amp;
  }
  for (; i < len; i++) {        // We missed a frame...
    stream[i] = stream[i - 1];  //... repeat last tone (I'm not sure why this
                                // works so well, but it does)
  }

  printf("Buffer Size %d!\n", lastS - lastP);
  printf("LastP: %i", lastP);
  printf("LastS: %i", lastS);
...
void update(double dt, int count = 1, bool audio = false) {
  if (lastS - lastP > 2 * 2048 || lastS + count >= buffSize)
  ...
  for (int z = 0; z < count; z++) {
    for (int q = 0; q < 10; q++) {
      ...
      for (int i = 0; i < numV - 1;
           i++) {  // Compute new accelration F=ma + Hook's Law
    ...
    if (audio) {  // Allow us to turn off the sound (for debugging)
      SDL_LockAudio();
      soundBuff[lastS++] = .5 * velY[numV / 2] + .25 * velY[numV / 2 - 10] +
                           .25 * velY[numV / 2 + 10];
      SDL_UnlockAudio();
    }
...
void printState() {
  printf("tension: %f   length: %f    K: %f   kV: %f\n",
         restLen * numV / (posX[numV - 1] - posX[0]), posX[numV - 1] - posX[0],
         k, kV);
}
...
int main(int argc, char* argv[]) {
  ...
  SDL_Window* window = SDL_CreateWindow("My OpenGL Program", 100, 100, 800, 400,
                                        SDL_WINDOW_OPENGL);
  ...
  while (true) {
    ...
    glVertexAttribPointer(posAttrib, 2, GL_DOUBLE, GL_FALSE, 5 * sizeof(double),
                          0);
    ...
    glVertexAttribPointer(colAttrib, 3, GL_DOUBLE, GL_FALSE, 5 * sizeof(double),
